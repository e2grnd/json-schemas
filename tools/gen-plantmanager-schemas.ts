...
program
  ...
  .option(
    "--dry-run",
    "Print generated JSON to stdout instead of writing files",
  )
  .option(
    "--no-dereference",
 ...
 */
function parseEnumDescription(description: string): Map<number, string> | null {
...
async function main(): Promise<void> {
  ...
  console.error(
    `Built enum registry — ${Object.keys(enumRegistry).length} enum types`,
  )
  ...
  if (!opts.dereference) {
    ...
    if (opts.dryRun) {
      process.stdout.write(
        `// === enum schemas (${enumEntries.length} total) ===\n\n`,
      )
    } else {
    ...
    for (const [enumName, enumSchema] of enumEntries) {
      ...
      if (opts.dryRun) {
        process.stdout.write(
          `// --- ${enumName} (enums/${kebabName}.schema.json) ---\n`,
        )
        process.stdout.write(json)
        process.stdout.write("\n")
      } else {
